...
impl EventAbiDecoder {
    ...
    pub fn decode_event(
        ...
        for input in &event.inputs {
            if input.indexed {
                ...
                if topic_idx < topics.len() {
                    let value =
                        Self::decode_indexed_param(&input.ty, &topics[topic_idx], monitor_name)?;
        ...
        if !data_types.is_empty() {
            let decoded_data = Self::decode_data_params(&data_types, data, monitor_name)?;
    ...
    fn decode_indexed_param(param_type: &str, topic: &Bytes, monitor_name: &str) -> Result<Value> {
        // Parse the type
        let sol_type =
            param_type
                .parse::<DynSolType>()
...
mod tests {
    ...
    fn test_decode_event_with_string() {
        let event = EventAbiDecoder::parse_event("Message(address indexed sender, string message)")
            .unwrap();
        ...
        let decoded =
            EventAbiDecoder::decode_event(&event, &topics, &Bytes::from(data), "test").unwrap();
    ...
    fn test_decode_event_with_array() {
        let event =
            EventAbiDecoder::parse_event("Winners(address[] winners, uint256[] amounts)").unwrap();
    ...
    fn test_decode_event_with_tuple() {
        ...
        let data = (Address::from([0xAA; 20]), U256::from(1000), true).abi_encode();
    ...
    fn test_decode_indexed_string() {
        ...
        let event =
            EventAbiDecoder::parse_event("NameSet(string indexed name, address setter)").unwrap();
